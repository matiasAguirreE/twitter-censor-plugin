...
def train_old_model():
    """Train the OLD model using original data without augmentation"""
    print("🏛️  Starting training for OLD model...")
    print("📊 Using hyperparameters optimized for data WITHOUT augmentation:")
    ...
    print("📂 Loading original dataset (without data augmentation)...")
    df = pd.read_csv(DATA_PATH)
    print(f"📊 Dataset shape: {df.shape}")
    print(f"📈 Original data: {df.shape[0]} tweets (NO augmentation)")
    ...
    print("🤖 Initializing OLD model with optimal hyperparameters...")
    ...
    print("🎉 OLD model training completed successfully!")
    print(f"📁 Model saved to: {MODEL_PATH}")
    print(f"📁 Tokenizer saved to: {ARTIFACTS_DIR}")
    print(f"🚀 Trained on: {device_name}")
    print("📊 This model was trained with hyperparameters optimized for:")
    print("   - Original data (1,000 tweets)")
    print("   - No data augmentation")
    print("   - Expected F1-Macro: ~82.6%")
